...
const StaffLayout = () => {
    ...
    const sidebarItems = [
        ...
        { label: 'Heroes Roster', icon: User, path: '/staff/roster' }, // Using User for singular hero focus? User asked for UserGroup. I don't have UserGroup imported. Let's use User or Users.
        { label: 'Team Directory', icon: Users, path: '/staff/directory' },
    ...
    return (
        <div className="flex h-screen bg-slate-50 text-slate-900 font-sans">
            ...
            >
                <div className="relative z-10 bg-transparent px-3 pt-3">
                    <Link to="/staff/dashboard">
                        <SaaSBrandHeader theme="light" />
                    </Link>

                    {/* Floating Toggle Button (Edge) */}
                    <button
                        onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
                        className="absolute -right-3 top-9 z-50 bg-white border border-slate-200 rounded-full p-1 shadow-md hover:bg-slate-50 text-slate-500 hover:text-emerald-600 transition-colors"
                    >
